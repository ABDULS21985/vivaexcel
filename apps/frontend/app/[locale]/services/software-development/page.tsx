...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!tower) {
    return { title: "Software Development | Global Digitalbit" };
  }
  return {
...
export default async function SoftwareDevelopmentPage({ params }: Props) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#f8f5ff] via-white to-white">
          ...
          <div className="grid lg:grid-cols-[1.15fr_0.85fr] gap-12 items-center">
            <div className="space-y-6">
              ...
              <h1 className="text-4xl md:text-5xl font-bold text-neutral-900 leading-tight">
                ...
                <span className="text-neutral-700"> &mdash; ship production-grade software, fast and secure</span>
              </h1>
              ...
              <div className="grid sm:grid-cols-3 gap-4 max-w-3xl">
                <StatPill icon={<Code2 size={18} />} label="Stack agnostic" value="React, Go, Python, .NET+" accent={accent} />
                <StatPill icon={<Rocket size={18} />} label="DORA elite" value="Deploy daily, MTTR <1h" accent={accent} />
                <StatPill icon={<Zap size={18} />} label="Shift-left security" value="SAST/DAST in every PR" accent={accent} />
              </div>
            </div>
            <div id="quiz">
              {rc?.readinessQuiz && (
      ...
      {/* Exec Summary + Diagram */}
      <section className="container mx-auto px-4 md:px-6 lg:px-10 py-14">
        <div className="grid lg:grid-cols-[1fr_1fr] gap-10 items-start">
          <div className="space-y-6">
            <SectionHeader title="Why Engineering Matters" accent={accent} />
          ...
          <div>
            {rc?.diagramLayers && (
              <EngineeringStackDiagram layers={rc.diagramLayers} accentColor={accent} />
            )}
          </div>
        </div>
      </section>
      ...
      {rc?.pdca && rc.pdca.length > 0 && (
        <section className="bg-neutral-50 py-14">
          <div className="container mx-auto px-4 md:px-6 lg:px-10">
            <SectionHeader title="Plan \u2192 Build \u2192 Test \u2192 Ship" accent={accent} subtitle="A closed-loop engineering process that delivers quality at speed." />
        ...
        <div className="grid lg:grid-cols-[1.15fr_0.85fr] gap-10">
          {rc?.complianceRoadmap && (
            <div className="bg-white border border-neutral-100 rounded-2xl shadow-xl p-6">
              <SectionHeader title="Delivery Roadmap" accent={accent} />
          ...
          {rc?.estimator && <TimeToDeliveryEstimator estimator={rc.estimator} accentColor={accent} />}
        </div>
      </section>
      ...
      {rc?.faqs && rc.faqs.length > 0 && (
        <section className="container mx-auto px-4 md:px-6 lg:px-10 py-14">
          <SectionHeader title="Software Development FAQ" accent={accent} subtitle="What CTOs, product owners, and engineering leads ask most." />
      ...
      <section className="bg-neutral-50 py-14">
        <div className="container mx-auto px-4 md:px-6 lg:px-10">
          <SectionHeader title="What We Deliver" accent={accent} subtitle="End-to-end engineering services from architecture to production." />
      ...
      <CTASection
        title="Ready to build software that scales?"
        description="Schedule a working session with our architects to scope your project and define the fastest path to production."
        primaryCTA={{ label: "Schedule architecture session", href: `/contact?service=${slug}` }}
        secondaryCTA={{ label: "Download engineering playbook", href: "/contact?type=asset&name=engineering-playbook" }}
      />
    </div>
  );
}
